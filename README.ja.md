...
**v1.4.0 短編執筆と Studio Chat ワークフロー更新** — Studio Chat と CLI から独立した短編パッケージ、あらすじ、セールスポイント、表紙プロンプト、任意の表紙画像を生成できます。プロジェクトチャットはリロード後も保持され、生成物のプレビューと編集にも対応しました。Studio のモデル設定では、集約モデルサービス向けに kkaiapi と OpenRouter を選択できます。
